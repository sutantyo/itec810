...
if(isset($opts->a)) {
    ...
    My_Logger::log(print_r($request_actions, true));
    ...
    if(isset($opts->p)) {
        ...
        My_Logger::log('p:' . $opts->p . ', parameters: ' . print_r($output, true) );
...
function cronlog($message, $severity = 0) {
    ...
    if (defined('STARTED')) My_Logger::log($message); //Use our logger only if application has been initialized (hardcoded define for now)
